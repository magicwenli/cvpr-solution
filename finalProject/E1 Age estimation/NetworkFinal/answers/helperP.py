...
#############################################################################################
def evaluate_hidden(w, b, age, feature):
    x = np.dot(feature, w[0]) + b[0].reshape(1,-1)
    x = np.maximum(x, 0)
    x = np.dot(x, w[1]) + b[1].reshape(1,-1)
    x = np.exp(x)
    prob = x / np.sum(x, axis=-1)[:, None]
    x = np.dot(prob,np.arange(0,101))
    loss = np.abs(x - age).mean()
...
def test_hidden(w, b, feature, filename='sgd_hidden.txt'):
    x = np.dot(feature, w[0]) + b[0].reshape(1,-1)
    x = np.maximum(x, 0)
    x = np.dot(x, w[1]) + b[1].reshape(1,-1)
    x = np.exp(x)
